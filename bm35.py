...
class bm35:
	...
	def bm35_parse_pressure_answer(answer):
		re_answer=re.compile('^M[0-9][0-9]D[0-9][0-9].[0-9][0-9].[0-9][0-9][0-9][0-9],[0-9][0-9]:[0-9][0-9],[0-9]+,[0-9]+,[0-9]$')
		if not(re_answer.match(answer)):
			return 'Invalid answer to parse'
		m=re.compile('[, || D || M || .]')
		splited=m.split(answer)
		the_date=splited[4]+'-'+splited[3]+'-'+splited[2]+' '+splited[5]+':00'
		return {'date':the_date,
			'meanPressure':splited[6],
			'instantPressure':splited[7]}

	@staticmethod
	def bm35_request_1min_reading_perid(port)
		command='A00I10'
