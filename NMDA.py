...
from counts_manager import CountsManager
...
def create_parser():
    ...
    try:
        ...
        vaisala = dict(config.items("Vaisala"))
        ...
        logging.info(
            'Probably the .NMDA.conf file is not configured. The NMDA.conf.example is an example file of how to config the .NMDA.conf file')
        print 'Exiting'
    ...
    parser = argparse.ArgumentParser(
        description="Launch the python module for the new pulse width core for the neutron monitors.")
    parser.set_defaults(**defaults)
    parser.add_argument('-sp', '--serial_port_control', help='The port that will be used to read the data.')
    parser.add_argument('-db', '--database',
                        help='The database where the data will be stored, by default the data will be printed on the shell.')
    parser.add_argument('-sps', '--serial_port_sensors', help='The port the sensors will use to deliver their data.')
    parser.add_argument('-bm', '--barometer_type', choices=['ap1', 'bm35'],
                        help='The barometer used for the pressure measurement.')
    parser.add_argument('-hv', '--hvps_type', choices=['digital', 'analog'],
                        help='Analog or Digital high voltage power suplly.')
    ...
    parser.add_argument('-dbU', '--db_updater_enabled',
                        help='Enables or disables the remote databese updater(DBUpdater).')
...
def validate_arguments(args):
    ...
    # Valid bar_type
    if not (args.barometer_type is None or args.barometer_type == 'ap1' or args.barometer_type == 'bm35' or
            args.barometer_type == 'vaisala'):
        end('Invalid barometer type')

    if args.barometer_type == 'vaisala':
        if args.vaisala_ip is None or args.vaisala_port is None:
            end('If barometer type is vaisala, you must specify vaisala_ip and vaisala_port')
    ...
    if (args.hvps_type == 'digital' or args.barometer_type == 'bm35') and (
            args.serial_port_control == None or args.serial_port_sensors == None):
...
def init_resources(args):
    ...
    port = init_port(args.serial_port_control, args_baudrate=9600)
    ...
    sensors_manager = SensorsManager(args)
...
def init_threads(port, args, port_sensors, conn, sensors_manager):
    ...
    dbUpConf = None
    if args.db_updater_enabled == True:
        dbUpConf = {'local': {'name': args.local_db},
                    'remote': {'host': args.remote_db_host, 'user': args.remote_db_user, 'pass': args.remote_db_pass,
                               'database': args.remote_db_db}}
        ...
        try:
            ...
            logging.info(
                'Could not correctly init the remote database, but  the data acquisition software will continue as expected. The software will anyway try to write the data to the remote database every minute.')
    ...
    counts = CountsManager(port, counts_condition, shared_counts, shared_countsFromEvents, shared_events, conn,
                           sensors_manager, dbUpConf, args.channel_avg, pressureConf, efficiencyConf)
...
if __name__ == '__main__':
    ...
    print "init_resources: done"

    k1 = FpgaK1SerialReader(port)
    manager = CountsManager(conn)
    sensors = SensorsManager(args)

    while True:
        time_to_sleep = 60 - (time.time() % 60)
        print "Sleeping {} seconds".format(time_to_sleep)
        time.sleep(time_to_sleep)
        print "Time is {}".format(time.strftime("%H:%M:%S"))
        values = k1.get_samples()
        k1.print_values(values)
        environment = {
            'hv1': 999,
            'hv2': 999,
            'hv3': 999,
        ...
        print environment
        manager.save_counts(time.time(), values, environment)
