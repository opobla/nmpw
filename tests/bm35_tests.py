...
class bm35TestCase(unittest.TestCase):
	def test_Equality_compute_crc(self):
		...
		self.assertEqual(response,'AAA195','Incorrect crc computed')
		
		response=bm35.bm35_compute_crc('HiThere')
		self.assertEqual(response,'HiThere681','Incorrect crc computed')

		response=bm35.bm35_compute_crc('aaa')
		self.assertNotEqual(response,'aaa195','Incorrest crc computed')
	...
	def test_Equality_parse_pressure_answer(self):
		...
		self.assertEqual(response,{'date':'1999-02-08 11:40:00','meanPressure':96502,'instantPressure':96555}, 'Answer parsed Incorrectly')

		response=bm35.bm35_parse_pressure_answer('M99D17.07.2014,12:47,85661,11555,0768')
		self.assertEqual(response,{'date':'2014-07-17 12:47:00','meanPressure':85661,'instantPressure':11555}, 'Answer parsed Incorrectly')
